  ...
  const legalText = useMemo(() => {
    ...
    return {
      corporate: {
        ...
        body:
          lang === "tr"
            ? "Üreten Eller kurumsal bilgi sayfası. Buraya şirket tanıtımı, misyon, vizyon ve marka politikaları gelecek."
            : lang === "en"
            ? "Corporate information page. Add company intro, mission, vision and brand policies here."
            : lang === "ar"
            ? "صفحة المعلومات المؤسسية. أضف المقدمة والرسالة والرؤية وسياسات العلامة هنا."
            : "Unternehmensseite. Hier Vorstellung, Mission, Vision und Markenrichtlinien."
      },
      about: {
        title: S.about,
        body:
          lang === "tr"
            ? "Hakkımızda metni için yer tutucu."
            : lang === "en"
            ? "About us placeholder."
            : lang === "ar"
            ? "نص من نحن (مكان الحفظ)."
            : "Über-uns Platzhalter."
      },
      contact: {
        title: S.contact,
        body:
          lang === "tr"
            ? "İletişim: info@ureteneller.com — Buraya adres ve telefon bilgileri ekleyin."
            : lang === "en"
            ? "Contact: info@ureteneller.com — Add address and phone here."
            : lang === "ar"
            ? "اتصال: info@ureteneller.com — أضف العنوان والهاتف هنا."
            : "Kontakt: info@ureteneller.com — Adresse und Telefon hier."
      },
      privacy: {
        title: S.privacy,
        body:
          lang === "tr"
            ? "Gizlilik Politikası için yer tutucu metin."
            : lang === "en"
            ? "Privacy Policy placeholder text."
            : lang === "ar"
            ? "نص سياسة الخصوصية (مكان حفظ)."
            : "Datenschutz-Richtlinie Platzhalter."
      },
      kvkk: {
        title: S.kvkk,
        body:
          lang === "tr"
            ? "KVKK Aydınlatma Metni için yer tutucu."
            : lang === "en"
            ? "KVKK Notice placeholder."
            : lang === "ar"
            ? "نص إشعار KVKK (مكان حفظ)."
            : "KVKK-Hinweis Platzhalter."
      },
      privacyTerms: {
        title: S.privacyTerms,
        body:
          lang === "tr"
            ? "Gizlilik & Kullanım için özet yer tutucu."
            : lang === "en"
            ? "Privacy & Terms summary placeholder."
            : lang === "ar"
            ? "ملخص الخصوصية والشروط (مكان حفظ)."
            : "Kurzfassung Datenschutz & AGB (Platzhalter)."
      },
      terms: {
        title: S.terms,
        body:
          lang === "tr"
            ? "Kullanım Şartları metni için yer tutucu."
            : lang === "en"
            ? "Terms of Use placeholder."
            : lang === "ar"
            ? "نص شروط الاستخدام (مكان حفظ)."
            : "Nutzungsbedingungen Platzhalter."
      },
      distance: {
        title: S.distance,
        body:
          lang === "tr"
            ? "Mesafeli Satış Sözleşmesi için yer tutucu."
            : lang === "en"
            ? "Distance Sales Agreement placeholder."
            : lang === "ar"
            ? "اتفاقية البيع عن بُعد (مكان حفظ)."
            : "Fernabsatzvertrag Platzhalter."
      },
      shippingReturn: {
        title: S.shippingReturn,
        body:
          lang === "tr"
            ? "Teslimat ve İade Politikası için yer tutucu."
            : lang === "en"
            ? "Shipping & Returns Policy placeholder."
            : lang === "ar"
            ? "سياسة الشحن والإرجاع (مكان حفظ)."
            : "Liefer- & Rückgabebedingungen Platzhalter."
      },
      cookies: {
        title: S.cookies,
        body:
          lang === "tr"
            ? "Çerez Politikası için yer tutucu."
            : lang === "en"
            ? "Cookie Policy placeholder."
            : lang === "ar"
            ? "سياسة ملفات تعريف الارتباط (مكان حفظ)."
            : "Cookie-Richtlinie Platzhalter."
      },
      help: {
        title: S.help,
        body:
          lang === "tr"
            ? "Yardım / SSS metni için yer tutucu."
            : lang === "en"
            ? "Help / FAQ placeholder."
            : lang === "ar"
            ? "مساعدة / أسئلة شائعة (مكان حفظ)."
            : "Hilfe / FAQ Platzhalter."
      },
      banned: {
        title: S.banned,
        body:
          lang === "tr"
            ? "Yasaklı Ürünler listesi için yer tutucu."
            : lang === "en"
            ? "Prohibited Products list placeholder."
            : lang === "ar"
            ? "قائمة المنتجات المحظورة (مكان حفظ)."
            : "Liste verbotener Produkte (Platzhalter)."
      },
      all: {
        title: S.all,
        body:
          lang === "tr"
            ? "Tüm yasal belgeleri bu sayfada toplayabilirsiniz."
            : lang === "en"
            ? "You can consolidate all legal documents here."
            : lang === "ar"
            ? "يمكن جمع جميع الوثائق القانونية هنا."
            : "Hier können alle Rechtstexte gebündelt werden."
      }
    };
  }, [lang]);
  return legalText;
...
export default function Home() {
  ...
  return (
    <main className="wrap">
      ...
      {/* ===== Siyah Legal Alt Panel (tam genişlik) ===== */}
      <footer className="legalFooter" role="contentinfo">
        <div className="legalWrap">
          ...
          <nav className="legalLinks" aria-label={(STR[lang] || STR.tr).legalBarTitle}>
            {/* Soldan sağa tüm linkler, boşluk bırakmadan */}
      ...
      {/* Modal / Drawer: Linke basınca dosya açılır */}
      {openLegal && (
        <div className="modal" role="dialog" aria-modal="true" aria-label={LEGAL_TEXT[openLegal]?.title || "Legal"}>
          <div className="modalCard">
            ...
            <div className="modalBody">
              <p>{LEGAL_TEXT[openLegal]?.body}</p>
            </div>
            <footer className="modalFoot">
              <button className="closeFull" onClick={() => setOpenLegal("")}>{LEGAL_LABELS.close}</button>
            </footer>
          </div>
      ...
      <style jsx global>{`
        ...
        .wrap {
          max-width: 1120px;
          margin: 0 auto;
          padding: 32px 20px 120px; /* alt legal panel için ekstra boşluk */
        }
        ...
        .logo { filter: drop-shadow(0 10px 24px rgba(0, 0, 0, 0.18)); border-radius: 20px; }
        .title, .subtitle { transition: color 0.6s ease; }
        .title { margin: 8px 0 0; font-size: 48px; color: var(--accent); }
        .subtitle { margin: 2px 0 6px; font-size: 24px; color: var(--accent); }
        .lead { max-width: 820px; margin: 8px auto 4px; font-size: 18px; color: var(--accent); transition: color 0.6s ease; }
        .phrase { animation: fadeSlide 0.7s ease; }
        @keyframes fadeSlide { from { opacity: 0; transform: translateY(6px);} to { opacity: 1; transform: none;} }

        .ctaRow { display: flex; gap: 12px; flex-wrap: wrap; justify-content: center; margin-top: 8px; }
        .btnPrimary { padding: 12px 18px; border-radius: 999px; border: none; cursor: pointer; background: #111827; color: #fff; font-weight: 600; box-shadow: 0 8px 24px rgba(0,0,0,0.15); }
        ...
        .adCard { background: #fff; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden; display: flex; flex-direction: column; box-shadow: 0 6px 18px rgba(0,0,0,0.06); }
        ...
        .card { border-radius: 18px; padding: 16px; background: var(--paperA); border: 1px solid var(--lineA); backdrop-filter: blur(8px); box-shadow: 0 12px 28px rgba(0,0,0,0.08); transition: transform .2s ease, box-shadow .2s ease, filter .5s linear; cursor: pointer; }
        ...
        /* ===== Siyah Legal Alt Panel ===== */
        .legalFooter { position: fixed; left: 0; right: 0; bottom: 0; background: #0b0b0b; color: #f8fafc; border-top: 1px solid rgba(255,255,255,0.12); z-index: 40; }
        ...
        .legalLinks > *:hover { background: rgba(255,255,255,0.08); color: #fff; }
        ...
        .modalCard { position: relative; background: #fff; width: min(920px, 92%); max-height: 85vh; border-radius: 16px 16px 0 0; box-shadow: 0 -10px 30px rgba(0,0,0,0.25); padding: 12px 14px 14px; overflow: hidden; }
